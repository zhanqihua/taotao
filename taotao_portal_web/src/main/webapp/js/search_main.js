...
SEARCH.relate_search = {html: null,init: function() {
        ...
        if ("string" == typeof (e) && e.length > 0) {
            ...
            var h = '<div class="related-service"><span>相关搜索：</span>';
...
SEARCH.get_book_summary = {fragment: ['<div class="img"><img src="{img}" width="280" height="280" alt="" /></div><div class="text"><table cellpadding="0" cellspacing="0" border="0" width="100%">{detaile}</table></div>', '<div class="summary"><div class="i-summary"><div class="close" onclick="$(this).parent().parent().hide()"></div><div class="arrow"></div><div class="con"><div class="iloading">\u6b63\u5728\u52a0\u8f7d\u4e2d\uff0c\u8bf7\u7a0d\u5019...</div></div></div></div>', "<tr><td>{name}{value}</td></tr>"],dataUrl: "http://d.360buy.com/bookExt/get?skuId=",edataUrl: "http://e.jd.com/ebook/ExteBookService.aspx?stype=service&skuid=",setPosition: function(a) {
...
SEARCH.get_shop_name = function() {
    var e = this, c = $("#plist").find("div.mapper,div.shop-name").not('[done="1"]').filter("[shop_id]"), d = c.length;
    if (!d) {
        return false
    }
    for (var b = 0, a = []; b < d; b++) {
...
SEARCH.bind_events = {iplocation: (function(e, k) {
        ...
        var j = $("#brand-service"), h = g.find("div.tabcon"), i = h.children("div"), c = h.hasClass("show-logo"), b = window.pageConfig.wideVersion ? c ? 14 : 15 : c ? 10 : 12, l = g.find("ul.tab"), f = "可搜索拼音、汉字查找品牌", d = false, e;
        ...
        var d = $("#text-stock-service"), c = $("#btn-stock-service"), a = "在结果中搜索";
        function b() {
...
SEARCH.page = function(b, a) {
    ...
    window.location = "http://localhost:8086" +
        "" +
        "" +
        "" +
        "" +
        "" +
        "/service.html?q="+SEARCH.query+"&page="+b;
//    if (a) {
...
SEARCH.get_ware_stock = function(b, a) {
    $.getJSON("http://service.jd.com/stock?skus=" + b.replace(/J_/g, "") + "&district=" + a.split("-").slice(0, 3).join("_") + "&callback=?", function(c) {
...
SEARCH.get_ware_info_merge = function(e, c) {
    ...
    $.getJSONP("http://service.jd.com/mergesearch?skuids=" + b + "&apiType=1,2,4,8,16&area=" + c.split("-").slice(0, 3).join("_") + "&callback=SEARCH.jsonpcallback");
    setTimeout(function() {
...
SEARCH.get_baby_info = function() {
    ...
    function d(p) {
        ...
        if (l.status == "now") {
            j.find(".tc-info").html('您的宝宝<strong>即将出生</strong>啦！<a href="javascript:;" class="tci-change" clstag="service|keycount|service|updatebabyinfo">[ 修改信息 ]</a>')
        } else {
            if (l.status == "past") {
                j.find(".tc-info").html("您的宝宝已经<strong>" + l.tip + '</strong>啦！<a href="javascript:;" class="tci-change" clstag="service|keycount|service|updatebabyinfo">[ 修改信息 ]</a>')
            } else {
                if (l.status == "future") {
                    j.find(".tc-info").html("您的宝宝即将在<strong>" + l.tip + '</strong>后出生！<a href="javascript:;" class="tci-change" clstag="service|keycount|service|updatebabyinfo">[ 修改信息 ]</a>')
                }
            }
        }
...
SEARCH.get_paipai_ware = function(c) {
    ...
    $.ajax({url: "http://sse1.paipai.com/comm_json?charset=utf-8&dtype=jsonp&dtag=service&KeyWord=" + b + "&PageSize=5&ShowClass=1",dataType: "jsonp",scriptCharset: "utf-8",success: function(h) {
