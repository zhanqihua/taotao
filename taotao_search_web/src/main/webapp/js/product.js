...
function setSearch(t, e) {
    ...
    $.ajax({url: "http://api.search.jd.com/?key=" + s + "&pagesize=" + i + "&filt_type=wyn,L1M1;wstate,L1M1" + e,dataType: "jsonp",success: function(t) {
            var e = t.Summary.key;
            pageConfig.product.searchList = [], null !== t.Product && t.Product.length > 0 && (1 > $("#service-result").length && $("#product-intro").parent().before('<div class="w"><div id="service-result" class="m m2" clstag="shangpin|keycount|product|exrs"><div class="mt"></div><div class="mc service-item"></div></div></div>'), $("#service-result").show(), $("#service-result .service-item").html(search_TPL.process(t)), $("#service-result .mt").html('<h2>\u5728\u4eac\u4e1c\u5546\u57ce\u4e2d\u67e5\u770b\u5176\u5b83\u201c<a target="_blank" href="http://service.jd.com/Search?keyword=' + e + '">' + e + '</a>\u201d\u7684\u641c\u7d22\u7ed3\u679c\uff1a</h2><div class="extra"><a target="_blank" href="http://service.jd.com/Search?keyword=' + e + '">\u67e5\u770b\u66f4\u591a\u641c\u7d22\u7ed3\u679c</a></div>'), G.getPriceNum(pageConfig.product.searchList, null, $("#service-result")), $(".service-item dl .p-img a").click(function() {
            ...
            }), $(".service-item dl .p-name a").click(function() {
                var t = $(this).parents("dl").attr("skuid");
                $.getScript("http://sstat.jd.com/scslog?args=1^" + encodeURIComponent(e) + "^^1^^" + encodeURIComponent(document.referrer) + "^" + t + "^0^101^^0^0")
            }))
        }})
...
function onlineService(t, e, i) {
    ...
    var v = "http://" + i + "/index.action?", b = {pid: a,price: g,stock: l,score: d,commentNum: p,imgUrl: h,wname: n,advertiseWord: o,seller: e,evaluationRate: u,recent: f,code: t,area: s($("#store-selector .text").text()),size: s($("#choose-version .dd .selected a").text()),services: s($("#summary-mapper .dd").html())};
...
//                    $("#online-mapper").show(), $("#j-im").attr("title", i + " \u8054\u7cfb\u5ba2\u670d"), $("#j-im").click(function() {
...
//                } else
//                    (3 == s || 9 == s) && ($("#online-mapper").show().find("b").html("\u8054\u7cfb\u5ba2\u670d"), $("#j-im").addClass("d-offline").html("<b>\u7559\u8a00\u54a8\u8be2</b>").attr("title", i + " \u5ba2\u670d\u76ee\u524d\u4e0d\u5728\u7ebf\uff0c\u60a8\u53ef\u4ee5\u70b9\u51fb\u6b64\u5904\u7ed9\u5546\u5bb6\u7559\u8a00\uff0c\u5e76\u5728\u3010\u6211\u7684\u4eac\u4e1c->\u6d88\u606f\u7cbe\u7075\u3011\u4e2d\u67e5\u770b\u56de\u590d").click(function() {
...
//            }}) : 6728 == G.cat[0] && G.isJd ? $.ajax({url: "http://misc.360buyimg.com/product/js/2012/fittingsAuto.js?ver=20140424",dataType: "script",cache: !0}) : $.getJSONP("http://d.360buy.com/fittingInfo/get?skuId=" + t + "&callback=Recommend.cbRecoFittings"), void 0)
        ...
        $.ajax({url: "http://rs.jd.com/collocationBuy/mapper.jsonp?sku=" + t + "&venderId=" + e,dataType: "jsonp",success: function(t) {
        ...
        i.show(), e.show(), "#th-mapper" != t ? e.trigger("click") : a || e.trigger("click")
...
var Refresh = {init: function() {
        ...
        var s = "http://service.jd.com/sayword?", i = i || 0, t = t || G.orginSku, a = this;
        ...
        var e = $("#online-mapper");
        t && (t.companyName || t.firstAddr || t.secAddr) && (e.after('<dl id="pop-company"><dt>\u516c\u53f8\u540d\u79f0\uff1a</dt><dd></dd></dl><dl id="pop-address"><dt>\u6240&nbsp;\u5728&nbsp;\u5730\uff1a</dt><dd></dd></dl>'), $("#pop-company dd").html(t.companyName), $("#pop-address dd").html(t.firstAddr + "&nbsp;" + t.secAddr), pageConfig.wideVersion && pageConfig.compatible && $("#online-mapper dt").css("margin-bottom", 10))
        ...
        this.sku = t, this.resObj = {}, this.currSku = null, this.obj = e || $("#choose-mapper .dd"), this.fn = i || function() {
        }, this.url = "", this.typeMap = {t2: "ycbs",t3: "ycbs",t4: "ycbs",t6: "ywbh",t7: "ywbh",t9: "yhdx"}, this.itemTPL = '<div class="item">    {if arguments[2][0]&&arguments[2][1]==0}        <ins class="yb-item-more">\u66f4\u591a</ins>    {/if}    <b></b>    <a class="yb-link" href="http://fuwu.jd.com" target="_blank"><i class="yb-ico"></i></a>    <a href="#none" class="yb-item {if arguments[2][0]&&arguments[2][1]==0} yb-item-first{/if}" id="yb-pid-${platformPid}" data-type="${sortId}" data-sku="${platformPid}" title="${G.showServiceDesc(sortId)}">${sortName} \uffe5${price}</a></div>', this.TPL = '<div class="mapper-type-yb">    {if r1.length>0}        <div class="yb-item-cat yb-item-cat-r1">            ${JdService.itemTPL.process(r1[0], [r1.length>1, 0])}            <div class="yb-item-hide">                {for item in r1}                    {if item_index!=0}${JdService.itemTPL.process(item, [r1.length>1])}{/if}                {/for}            </div>        </div>    {/if}    {if r2.length>0}        <div class="yb-item-cat yb-item-cat-r2">            ${JdService.itemTPL.process(r2[0], [r2.length>1, 0])}            <div class="yb-item-hide">                {for item in r2}                    {if item_index!=0}${JdService.itemTPL.process(item, [r2.length>1])}{/if}                {/for}            </div>        </div>    {/if}    {if r3.length>0}        <div class="yb-item-cat yb-item-cat-r3">            ${JdService.itemTPL.process(r3[0], [r3.length>1, 0])}            <div class="yb-item-hide">                {for item in r3}                    {if item_index!=0}${JdService.itemTPL.process(item, [r3.length>1])}{/if}                {/for}            </div>        </div>    {/if}    {if r4.length>0}        <div class="yb-item-cat yb-item-cat-r4">            ${JdService.itemTPL.process(r4[0], [r4.length>1, 0])}            <div class="yb-item-hide">                {for item in r4}                    {if item_index!=0}${JdService.itemTPL.process(item, [r4.length>1])}{/if}                {/for}            </div>        </div>    {/if}    {if r5.length>0}        <div class="yb-item-cat yb-item-cat-r5">            ${JdService.itemTPL.process(r5[0], [r5.length>1, 0])}            <div class="yb-item-hide">                {for item in r5}                    {if item_index!=0}${JdService.itemTPL.process(item, [r5.length>1])}{/if}                {/for}            </div>        </div>    {/if}    <div class="yb-item-cat yb-item-detail"><a class="hl_blue" target="_blank" href="http://sale.jd.com/act/1pChm0RcanBPYXIS.html?erpad_source=erpad">\u8be6\u60c5</a></div></div>', this.yb_TPL = '        <div class="mapper-type-yb">            {for data in list}            <div class="yb-item-cat ${data.cName}">                <div class="item">                    {if data.items.length > 1}<ins class="yb-item-more">\u66f4\u591a</ins>{/if}                    <b></b>                    <a class="yb-link" href="http://fuwu.jd.com" target="_blank"><i class="yb-ico"></i></a>                    <a href="#none" class="yb-item {if data.items.length > 1} yb-item-first{/if}" id="yb-pid-${data.items[0].platformPid}" data-type="${data.items[0].sortId}" data-sku="${data.items[0].platformPid}" title="${G.showServiceDesc(data.items[0].sortId)}">${data.items[0].sortName} \uffe5${data.items[0].price}</a>                </div>                {if data.items.length > 1}                <div class="yb-item-hide">                    {for yb in data.items}                    {if Number(yb_index)>0}                    <div class="item">                        <b></b>                        <a class="yb-link" href="http://fuwu.jd.com" target="_blank"><i class="yb-ico"></i></a>                        <a href="#none" class="yb-item" id="yb-pid-${yb.platformPid}" data-type="${yb.sortId}" data-sku="${yb.platformPid}" title="${G.showServiceDesc(yb.sortId)}">${yb.sortName} \uffe5${yb.price}</a>                    </div>                    {/if}                    {/for}                </div>                {/if}            </div>            {/for}            <div class="yb-item-cat yb-item-detail">                <a class="hl_blue" target="_blank" href="http://sale.jd.com/act/1pChm0RcanBPYXIS.html?erpad_source=erpad">\u8be6\u60c5</a>            </div>        </div>', this.get()
    },bindEvent: function() {
        var t = this;
        this.obj.find(".mapper-type-yb .item a").bind("click", function() {
        ...
        var e = t.parents(".mapper-type").eq(0), i = e.find(".item");
...
//    G.isJd && "enabled" == pageConfig.btnBaiTiaoFq && t.after(e)
    ...
    pageConfig.product.havestock === void 0 ? (s = "", a = "http://cart.gift.jd.com/cart/addGiftToCart.action?pid=" + pageConfig.product.skuid + "&pcount=" + $("#buy-num").val() + "&ptype=1") : (s = pageConfig.product.havestock ? "" : "disabled", pageConfig.product.havestock ? (s = "", a = "http://cart.gift.jd.com/cart/addGiftToCart.action?pid=" + pageConfig.product.skuid + "&pcount=" + $("#buy-num").val() + "&ptype=1") : (s = "disabled", a = "#none")), 6980 == G.cat[2] && $('#product-detail [data-widget="tab-item"]').eq(3).hide(), $("#online-mapper").find("b").html("\u8054\u7cfb\u5ba2\u670d"), $("#comments-list li").eq(4).html('<a href="javascript:;">\u6709\u6652\u5355\u7684\u8bc4\u4ef7<em>(0)</em></a>'), $("body").addClass("cat-1-" + G.cat[0] + " cat-2-" + G.cat[1] + " cat-3-" + G.cat[2] + " item-" + G.sku), G.onAttr("HYKHSP-2", function() {
        ...
        if ("disabled" == pageConfig.serviceYB)
            return !1;
        $("#choose-result").length > 0 ? $("#choose-result").before('<li id="choose-mapper" class="hide"><div class="dt">\u4eac\u4e1c\u670d\u52a1\uff1a</div><div class="dd"></div></li>') : $("#choose-btns").length > 0 && $("#choose-btns").before('<li id="choose-mapper" class="hide"><div class="dt">\u4eac\u4e1c\u670d\u52a1\uff1a</div><div class="dd"></div></li>');
        var t = $("#choose-btn-append a"), e = t.attr("href"), i = $("#choose-result .dd"), s = $("#choose-result .dd").html(), a = !1;
        JdService.init(G.sku, $("#choose-mapper .dd"), function(n) {
...
//                }
    ...
    (function() {
       ...
       // pageConfig.disabledSeoBanner || (t.push("2_163_5395"), t.push("2_163_5394")), 1315 === G.cat[0] && $("#Ad2_100_2272").length > 0 && t.push("2_100_2272"), G.isPop && t.push("2_601_3951"), $.ajax({url: "http://nfa.jd.com/loadFa_toJson.js?aid=" + t.join("-") + "&ver=20131107",dataType: "script",cache: !0}), insertScript("http://static.360buyimg.com/gemini/gemini/gemini_v2.2.1.js", {id: "1399174259281",name: "gemini_divide_script",charset: "utf-8",divideType: "MODULE_INJECT",param: "[{t:'xiaoyumi'}]"})
        ...
        var t = $("#consult-service .form"), e = $("#consult"), i = pageConfig.wideVersion && pageConfig.compatible, s = '            <div id="consult-wrap" class="consult-service">                <b></b>                <div class="consult-pop">                    <input type="text" id="txbReferSearch" placeholder="\u8bf7\u8f93\u5165\u5173\u952e\u8bcd" />                    <input type="button" value="\u641c\u7d22" id="btnReferSearch" class="btn-service" clstag="shangpin|keycount|product|consult9">                    <a class="consult-close hl_blue" href="#none" onclick="closeCounsultSearch()">\u53d6\u6d88</a>                </div>            </div>';
        ...
        }, i || (e.find(".consult-service").hover(function() {
            var t = $(this);
            setTimeout(function() {
                t.addClass("hover")
            }, 500)
