...
!function() {
    ...
    var a = $("#mapper-2013 a[href='http://en.360buy.com/']");
...
!function() {
    pageConfig.FN_ImgError(document), $("img[data-lazyload]").Jlazyload({type: "image",placeholderClass: "err-product"}), category.FN_Init(), document.getElementById("shortcut") ? $("#shortcut .menu").Jdropdown({delay: 50}) : ($("#biz-mapper").Jdropdown({delay: 50}, function() {
...
//    };
//    var c = '<a style="color:#005AA0" onclick="$o.del(event)">\u5220\u9664</a>', d = "\u641c\u7d22\u5386\u53f2", e = '<li id="d_#{id}" suggest-pos="#{suggest_pos}" title="#{title}" onclick="$o.clickItem(this)" history="1"><div class="service-item" style="color:#005AA0">#{keyword}</div><div class="service-count">' + d + "</div></li>", f = '<li id="d_#{id}" suggest-pos="#{suggest_pos}" title="#{title}" onclick="$o.clickItem(this)"><div class="service-item">#{keyword}</div><div class="service-count">\u7ea6#{amount}\u4e2a\u5546\u54c1</div></li>', g = '<div id="d_#{id}" suggest-pos="#{suggest_pos}" class="#{className}" title="#{title}" cid="#{cid}" cLevel="#{cLevel}" onclick="$o.clickItem(this)"><div class="service-item">\u5728<strong>#{cname}</strong>\u5206\u7c7b\u4e2d\u641c\u7d22</div><div class="service-count">\u7ea6#{amount}\u4e2a\u5546\u54c1</div></div>#{categorys}', h = '<li class="fore1"><div id="d_#{id}" suggest-pos="#{suggest_pos}" class="fore1" title="#{title}" onclick="$o.clickItem(this)"><div class="service-item">#{keyword}</div><div class="service-count" #{style}>\u7ea6#{amount}\u4e2a\u5546\u54c1</div></div>#{categorys}</li>', i = "http://dd.search.jd.com/?key=#{keyword}", j = "#FFDFC6", k = "#FFF", l = $("#key"), m = $("#shelper");
...
//        service.cid = null != b && "" != b ? b : null;
//        var c = a.getAttribute("cLevel");
//        service.cLevel = null != c && "" != c ? c : null;
//        var d = a.getAttribute("ev_val");
//        service.ev_val = null == d || $.trim(d).isEmpty() ? null : d;
//        var e = a.getAttribute("title");
//        null == e || $.trim(e).isEmpty() || l.val(e), service.additinal = "&suggest=" + a.getAttribute("suggest-pos"), this.click = a.id.substr(2) - m.find("li[history='1']").length, service("key")
//    }, a.prototype.mouseenter = function(a) {
//        var a = $(a);
//        a.attr("history") && a.find(".service-count").html(c), a.css("background", j);
//        var b = a.attr("id").split("_"), e = parseInt(b[1], 10);
//        if (e != this.index) {
//            if (this.index > -1) {
//                var f = $("#d_" + this.index);
//                f.css("background", k), f.attr("history") && f.find(".service-count").html(d)
//            }
//            this.index = e
//        }
//    }, a.prototype.mouseleave = function(a) {
//        a.css("background", k), a.attr("history") && a.find(".service-count").html(d)
//    }, a.prototype.selectItemNode = function(a) {
//        var b = this, e = $("#d_" + b.index + ":visible");
//        e.css("background-color", k), e.attr("history") && e.find(".service-count").html(d), -1 == b.index && -1 == a && (a = 0), b.index = (b.length + b.index + a) % b.length;
//        var f = $("#d_" + b.index);
//        if (f.length > 0) {
//            f.attr("history") && f.find(".service-count").html(c), f.css("background-color", j);
//            var g = f.attr("title");
//            null == g || $.trim(g).isEmpty() || l.val(g);
//            var h = f.attr("cid");
//            service.cid = null != h && "" != h ? h : null;
//            var i = f.attr("cLevel");
//            service.cLevel = null != i && "" != i ? i : null, service.ev_val = null, service.additinal = "&suggest=" + f.attr("suggest-pos"), b.click = b.index - m.find("li[history='1']").length
//        }
